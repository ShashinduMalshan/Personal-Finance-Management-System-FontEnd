          ...
          <div key={index} className="flex flex-col items-center flex-1 group">
            <div className="relative w-full flex items-end justify-center h-full">
              ...
              <div 
                ...
                className={`w-full max-w-[50px] rounded-t-xl transition-all duration-500 ease-out hover:brightness-110 ${item.color || 'bg-emerald-500'}`}
              ></div>
            </div>
            <span className="text-[10px] text-gray-400 dark:text-gray-500 mt-3 font-bold uppercase tracking-wider">{item.label}</span>
          </div>
        ))}
      </div>
    ...
    <div className="flex flex-col items-center justify-center gap-10 py-4">
      <div className="relative w-[220px] h-[220px]">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-3xl font-black text-gray-800 dark:text-white">${total.toLocaleString()}</span>
...
const Analytics: React.FC = () => {
  ...
  useEffect(() => {
    ...
    //   const sumInc = inc.data.reduce((acc, curr) => acc + curr.amount, 0);
    //   const sumExp = exp.data.reduce((acc, curr) => acc + curr.amount, 0);
  ...
  const trendData = [
    { label: 'Sep', value: 4200, color: 'bg-emerald-300' },
    { label: 'Oct', value: 3800, color: 'bg-emerald-400' },
    { label: 'Nov', value: 5200, color: 'bg-emerald-500' },
    { label: 'Dec', value: 4800, color: 'bg-emerald-400' },
    { label: 'Jan', value: 6100, color: 'bg-emerald-600' },
    { label: 'Feb', value: 5900, color: 'bg-emerald-500' },
  ];
    ...
    <div className="p-6 space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-gradient-to-br from-emerald-600 to-teal-700 rounded-[2rem] p-8 text-white relative overflow-hidden shadow-2xl shadow-emerald-200 dark:shadow-none">
          <div className="relative z-10">
            ...
            <h2 className="text-5xl font-black mb-4">${netSavings.toLocaleString()}</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-4">
        {[
            ...
            { label: 'Next Milestone', value: '$100k Net Worth', icon: Target, color: 'blue' },
        ].map((insight, i) => (
