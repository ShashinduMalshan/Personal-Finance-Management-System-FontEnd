...
const Insights: React.FC = () => {
  ...
  // 3. Analytics Engine (Memoized)
  const analytics = useMemo(() => {
    // If no expenses, we cannot generate insight data
    ...
    expenses.forEach(e => {
      ...
      // Check if date is valid before running logic
    ...
    const chartData: ChartDataPoint[] = Object.entries(categoryTotals).map(
      ([label, value]) => ({
        label,
        value,
        color: value > totalExpense * 0.3 ? '#f87171' : '#34d399'
      })
    );
  ...
  // 4. Debug Logger (Runs whenever analytics updates)
  useEffect(() => {
    if (analytics) {
      console.log("Updated Chart Data:", analytics.chartData);
    }
  }, [analytics]);
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-red-500 gap-4 min-h-[400px]">
        <AlertTriangle size={40} />
        <p>{error}</p>
        <button onClick={fetchData} className="px-4 py-2 bg-gray-100 rounded-lg text-gray-700 text-sm font-bold">Try Again</button>
      </div>
    );
  }
    ...
    <div className="p-8 space-y-8 animate-fade-in no-scrollbar max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 p-8 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-700">
          ...
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-4xl font-black text-gray-800 dark:text-white">
              {analytics?.weekendPercentage.toFixed(0) || 0}%
            </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          {!analytics || analytics.alerts.length === 0 ? (
             <div className="p-6 bg-gray-50 rounded-3xl text-gray-400 text-sm font-bold">No leaks detected this month. Good job!</div>
          ) : (
            ...
            <div key={alert.id} className="bg-white dark:bg-gray-800 rounded-3xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm flex gap-6 group hover:shadow-md transition-all">
              <div className={`shrink-0 w-12 h-12 rounded-2xl flex items-center justify-center ${alert.type === 'critical' ? 'bg-red-50 text-red-500' :
        ...
        <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] p-10 border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col">
          ...
          <div className="flex-1 min-h-[300px] flex items-center justify-center">
            {/* The Fix: Ensure analytics exists AND chartData has items */}
            {analytics && analytics.chartData && analytics.chartData.length > 0 ? (
              <SimpleBarChart data={analytics.chartData} />
            ) : (
              <div className="text-gray-400 font-medium text-sm">Not enough data to generate chart</div>
            )}
          </div>
